...
import { ExpenseCard } from "@/components/chat/expense-card";
import { IncomeCard } from "@/components/chat/income-card";
import { EmotionPicker } from "@/components/chat/emotion-picker";
...
export function ChatContent({ variant = "page" }: ChatContentProps) {
  ...
  const renderToolPart = (part: any) => {
    ...
    if (
      (part.type === "tool-registerExpense" || part.type === "tool-registerIncome") &&
  ...
  const activeLedgerOption = ledgerOptions.find((l) => l.type === activeLedger.type) ?? ledgerOptions[0];
          ...
          <div className={`flex flex-col items-center justify-center h-full px-6 ${isDrawer ? "py-6" : "pb-4"}`}>
            <div className="flex flex-col items-center gap-3 mb-6">
              <div className={`rounded-full bg-accent/30 flex items-center justify-center ${isDrawer ? "w-12 h-12" : "w-16 h-16"}`}>
                <HugeiconsIcon icon={Leaf01Icon} size={isDrawer ? 24 : 32} className="text-primary" strokeWidth={1.5} />
              </div>
      ...
      <div className={`px-4 pt-2 ${isDrawer ? "pb-3" : "pb-[calc(0.75rem+env(safe-area-inset-bottom))]"}`}>
        ...
        >
          <textarea
            ...
            placeholder={activeLedger.type === "business" ? "Registra una venta o gasto..." : "Escribe un mensaje..."}
            rows={1}
                ...
                <div className="absolute bottom-full left-0 mb-1.5 bg-white/90 backdrop-blur-xl rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-border/20 overflow-hidden min-w-32 z-50">
                  {ledgerOptions.map((opt) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setActiveLedger({ id: opt.id, type: opt.type, name: opt.label });
                        setLedgerOpen(false);
                      }}
                      className={`flex items-center gap-2 w-full px-3 py-2 text-left transition-colors cursor-pointer ${
                        activeLedger.type === opt.type ? "bg-primary/5" : "hover:bg-accent/20"
                      }`}
                    >
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center ${
                        activeLedger.type === opt.type ? "bg-primary text-primary-foreground" : "bg-accent/40"
                      }`}>
                        <HugeiconsIcon
                          icon={opt.icon}
                          size={11}
                          className={activeLedger.type === opt.type ? "text-primary-foreground" : "text-primary"}
                          strokeWidth={1.5}
                        />
                      </div>
                      <span className={`text-[11px] font-medium ${
                        activeLedger.type === opt.type ? "text-primary" : "text-foreground/70"
                      }`}>
                        {opt.label}
                      </span>
                    </button>
